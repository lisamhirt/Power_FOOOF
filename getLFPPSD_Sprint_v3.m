function [] = getLFPPSD_Sprint_v3(tmpSUB , Hemi , saveLOC)
...
for dtt = 1:height(trialTab)
    ...
    tempChan = trialTab.Ephys{dtt};
    nanLoc = find(isnan(tempChan));

    for dti = 1:length(nanLoc)

        changeVal = mean([tempChan(nanLoc(dti)-1)  tempChan(nanLoc(dti)+1)]);
        tempChan(nanLoc(dti)) = changeVal;

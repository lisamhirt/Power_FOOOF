function [] = getLFPPSD_Sprint_v2(tmpSUB , Hemi , saveLOC)
...
load(tempFile) % gets loaded as trialTab

% PSD for each channel by hemisphere
allSprintOut = double.empty; % output table
timeTab = double.empty;

for dtt = 1:height(trialTab)

    allChan = trialTab.Ephys{dtt};
    ...
    for ci = 1:height(allChan)
        ...
        allSprintOut{dtt}{ci,:} = sprintOUT;
        disp(['Trial',num2str(dtt)])

        % tempTime = allSprintOut{1,1}{ci,1}.channel.peaks;
...
% Save
cd(saveLOC)
splitName = split(matDirB,'_');
saveName = [splitName{1},'_', splitName{2},'_', splitName{3}, '.mat'];
save(saveName, "allSprintOut");

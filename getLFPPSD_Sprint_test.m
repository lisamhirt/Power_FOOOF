function [] = getLFPPSD_Sprint_test(tmpSUB, saveLOC)
...
%         rawLOC = 'E:\Dropbox\ASSFN_2024_SleepEEG';
%     otherwise
%         rawLOC = 'C:\Users\johna\Dropbox\ASSFN_2024_SleepEEG';
% end

% tmpSUB

% Hemi % this could be brain area / hemisphere
%% SPRiNT (without Brainstorm)
% Inputs
% Input time series
% STFT opts
opt.sfreq = 500;                    % Input sampling rate
opt.WinLength = 0.1;                  % STFT window length (this is one second) % was 1 originally 
opt.WinOverlap = 50;                % Overlap between sliding windows (in %)% was 50 before
...
opt.aperiodic_mode      = 'knee'; % alternative: knee
opt.peak_threshold      = 1.5;   % 2 std dev: parameter for interface simplification - LH: might change and 2 may be the default
% Matlab-only options
...
folderLOC = ['Y:\LossAversion\LH_tempSave\' ,  tmpSUB, '\SPOOOF_test\'];
cd(folderLOC)
...
tempFile = string(matDirB);

% lfpPARTS = split(lfpDataList,'_');
% lfpNIGHTS = lfpPARTS(:,:,3);
% lfpNIGHTSN = cellfun(@(x) str2double(x), lfpNIGHTS, 'UniformOutput',true);
% nightINDEX = ismember(lfpNIGHTSN,Hemi);
% nightPROC = lfpDataList{nightINDEX};

% cd(folderLOC)

% load(tempFile,'EphysBehavTABLE')

load(tempFile) % gets loaded as fTrial

% PSD for each channel by hemisphere
% cd(folderLOC)
% EphysBehavTABLE.SPRINT = cell(height(EphysBehavTABLE),1);

SPOOOFout = double.empty;

for dtt = 1:height(fTrial)
...
cd(saveLOC)
% parts = split(nightPROC,'_');
saveNAME = [tmpSUB,'winLength0.1_overlap50_SPRINT.mat'];
save(saveNAME,"SPOOOFout");
...
function [lfpDS_HP , refDS_HP] = getDS_HP(subLFP , subRef)

lfpArray = transpose(table2array(subLFP));
lfpArray2 = cellfun(@(x) transpose(x), lfpArray,...
    'UniformOutput',false);

refArray = transpose(table2array(subRef));
refArray2 = cellfun(@(x) transpose(x), refArray,...
    'UniformOutput',false);

lfpResamp = cellfun(@(x) resample(x, 500, 1375),...
    lfpArray2,'UniformOutput',false);
disp('Resample Done')

lfpDS_HP = cellfun(@(x) highpass(x,0.5,500),...
    lfpResamp,'UniformOutput',false);
disp('HighPass done')

refResamp = cellfun(@(x) resample(x, 500, 1375),...
    refArray2,'UniformOutput',false);
disp('Resample Done')

refDS_HP = cellfun(@(x) highpass(x,0.5,500),...
    refResamp,'UniformOutput',false);
disp('HighPass done')
